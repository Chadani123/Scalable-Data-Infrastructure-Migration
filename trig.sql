...
CREATE TABLE SalesTax (
    ...
    `state` CHAR(2),
    taxRegionName VARCHAR(64),
    estimatedCombinedRate DECIMAL(6,5),
    stateRate DECIMAL(6,5),
    estimatedCountyRate DECIMAL(6,5),
    estimatedCityRate DECIMAL(6,5),
    estimatedSpecialRate DECIMAL(6,5),
    riskLevel INT
) ENGINE=InnoDB;


-- Load data from 'TAXRATES.csv' into SalesTax table
...
LINES TERMINATED BY '\n'
IGNORE 1 ROWS
(`state`, @zip_str, taxRegionName, estimatedCombinedRate, stateRate, estimatedCountyRate, estimatedCityRate, estimatedSpecialRate, riskLevel)
SET zip = CAST(@zip_str AS DECIMAL(5,0));
...
DROP PROCEDURE IF EXISTS proc_UpdateProductBuyers;
DROP PROCEDURE IF EXISTS proc_UpdateCustomerPurchases;
DROP PROCEDURE IF EXISTS proc_UpdateSalesTax;
...
CREATE OR REPLACE PROCEDURE proc_UpdateSalesTax(IN p_order_id BIGINT UNSIGNED)
BEGIN
    ...
    -- Get tax rate
    SELECT estimatedCombinedRate INTO v_taxRate FROM SalesTax WHERE zip = v_zip;

    IF v_taxRate IS NULL THEN
        SET v_taxRate = 0.00;
    END IF;

    -- Calculate salesTax using standard rounding (up at $0.005, down below)
    -- using 0.000001 to ensure the correct rounding due to floating-point precision
    SET v_salesTax = ROUND(v_subtotal * v_taxRate + 0.000001, 2);
...
CREATE OR REPLACE PROCEDURE proc_UpdateCustomerPurchases(IN p_CustomerID BIGINT UNSIGNED)
BEGIN
    ...
    SELECT c.id AS 'customer number', 
           c.firstName AS fn, 
           c.lastName AS ln,
           GROUP_CONCAT(DISTINCT CONCAT(p.id, ' ', p.name) ORDER BY p.id SEPARATOR '|') AS products
    FROM Customer c
...
CREATE OR REPLACE PROCEDURE proc_UpdateProductBuyers(IN p_ProductID BIGINT UNSIGNED)
BEGIN
    ...
    SELECT p.id AS productID, 
           ...
           GROUP_CONCAT(DISTINCT CONCAT(c.id, ' ', c.firstName, ' ', c.lastName) ORDER BY c.id SEPARATOR ',') AS customers
    FROM Product p
...
CREATE OR REPLACE TRIGGER trg_Product_AfterUpdate
AFTER UPDATE ON Product
FOR EACH ROW -- mariadb only supports for each row
BEGIN
...
CREATE OR REPLACE TRIGGER trg_Orderline_BeforeInsert
BEFORE INSERT ON Orderline
FOR EACH ROW
BEGIN
    -- DECLARE must be at the beginning of the BEGIN...END block in MariaDB triggers, 
...
CREATE OR REPLACE TRIGGER trg_Orderline_BeforeUpdate
...
CREATE OR REPLACE TRIGGER trg_Orderline_AfterInsert
...
BEGIN
    ...
    UPDATE `Order`
    SET subtotal = IFNULL(subtotal, 0.00) + NEW.lineTotal -- subtotal + NEW.lineTotal
...
CREATE OR REPLACE TRIGGER trg_Orderline_AfterUpdate
...
BEGIN
    ...
    -- Update subtotal in Order
    UPDATE `Order`
    SET subtotal = IFNULL(subtotal, 0.00) + v_lineTotal_diff -- subtotal + v_lineTotal_diff
    WHERE id = NEW.order_id;
...
CREATE OR REPLACE TRIGGER trg_Orderline_AfterDelete
...
BEGIN
    ...
    UPDATE `Order`
    SET subtotal = IFNULL(subtotal, 0.00) - OLD.lineTotal -- subtotal - OLD.lineTotal
