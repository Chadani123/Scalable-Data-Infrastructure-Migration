...
-- SELECT 
--     id,
--     -- Convert Ordered datetime to DATE, handle zero and empty dates
--     CASE 
--         WHEN orderDate = '0000-00-00' OR orderDate = '' OR STR_TO_DATE(orderDate, '%Y-%m-%d') = '0000-00-00' THEN NULL
--         ELSE DATE(STR_TO_DATE(orderDate, '%Y-%m-%d %H:%i:%s'))  -- Extracting only the date part
--     END AS orderDate,
--     -- Convert Shipped datetime to DATE, set to NULL if "Cancelled" or improperly formatted
--     CASE 
--         WHEN dateShipped = '0000-00-00' OR dateShipped = '' OR dateShipped = 'Cancelled' OR STR_TO_DATE(dateShipped, '%Y-%m-%d') = '0000-00-00' THEN NULL
--         ELSE DATE(STR_TO_DATE(dateShipped, '%Y-%m-%d %H:%i:%s'))  -- Extracting only the date part
--     END AS dateShipped,
--     customerId
-- FROM 
--     TempOrder
